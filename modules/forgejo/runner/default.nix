...

{
  ...
  den.aspects.forgejo.provides.runner = {
    nixos =
      { pkgs, config, ... }:
      let
        ...
        tags = {
          ...
          "x86_64-linux" = "ubuntu-latest-amd64";
        };
      ...
      in
      {
        ...
        age.secrets = {
          forgejo-runner-nix-netrc = {
            file = ../../../agenix/forgejo/runner/nix-netrc.age;
            mode = "440";
          };

          forgejo-runner-token = {
            file = ../../../agenix/forgejo/runner/token.age;
          };
        };
        ...
        systemd.services = {
          forgejo-runner-nix-image = {
            ...
            script = ''
              ...
              substituters = https://nix-cache.kempkens.network/nifoc-ci?priority=1 https://nix-community.cachix.org?priority=50 https://cache.nixos.org/
              trusted-public-keys = nifoc-ci:YGmPyqh3kbF7eQhMX9esbQpdYU4f3Q+WEx+sv97KKHo= nix-community.cachix.org-1:mB9FSh9qf2dCimDSUo8Zy7bkq5CX+/rkCWyvRCYg3Fs= cache.nixos.org-1:6NCHdD59X431o0gWypbMrAURkbJ16ZPMQFGspcDShjY=
              NIX_CONFIG

              cp ${config.age.secrets.forgejo-runner-nix-netrc.path} etc/nix/netrc
        ...
        services.gitea-actions-runner = {
          ...
          instances =
            let
              ...
              tag = tags.${pkgs.stdenv.system};
              nixTag = nixTags.${pkgs.stdenv.system};
            in
            {
              act = {
                enable = true;
              ...
              nix =
                ...
                in
                {
                  ...
                  settings = {
                    container = {
                      options = "-e NIX_BUILD_SHELL=/bin/bash -e PAGER=cat -e PATH=/bin -e SSL_CERT_FILE=/etc/ssl/certs/ca-bundle.crt -v /nix:/nix -v ${storeDeps}/bin:/bin -v ${storeDeps}/etc/ssl:/etc/ssl --user nix-ci-user";
                      valid_volumes = [
                        "/nix"
                        "${storeDeps}/bin"
                        "${storeDeps}/etc/ssl"

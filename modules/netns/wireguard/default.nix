...

{
  ...
  den.aspects.netns.provides.wireguard = {
    ...
    nixos =
      ...
      let
        wgConfig = den.aspects.${config.networking.hostName}.meta.netns.wireguard;
      in
      {
        imports = [
        ...
        age.secrets = builtins.listToAttrs (
          builtins.map (
            { name, ... }:
            {
              name = "netns-wireguard-${name}-config";
              value = {
                file = ../../../agenix/netns/wireguard/${config.networking.hostName}/${name}-config.age;
              };
            }
          ) wgConfig
        );
            ...
            builtins.listToAttrs (
              lib.lists.flatten (
                builtins.map (
                  { name, veth, ... }:
                  let
                    configFile = config.age.secrets."netns-wireguard-${name}-config";
                  in
                  [
                    {
                      name = "wg-${name}";
                      ...
                      value = {
                        ...
                        serviceConfig =
                          let
                            ...
                            startScript = writeShScript "wg-${name}-firewall-start" (
                              ''
                            ...
                            stopScript = writeShScript "wg-${name}-firewall-stop" ''
                            ...
                            reloadScript = writeShScript "wg-${name}-firewall-reload" ''
                              ip46tables -D INPUT -j wg-drop 2>/dev/null || true
                              ip46tables -D OUTPUT -j wg-drop 2>/dev/null || true
              ...
              lib.lists.flatten (
                builtins.map (
                  ...
                  builtins.map (srv: {
                    ...
                    value = {
                      bindsTo = [ "wg-${name}.service" ];
                      after = lib.mkAfter [ "wg-${name}.service" ];
                      ...
                      serviceConfig = {
                        ...
                        BindReadOnlyPaths = [
                          "/etc/netns/${name}/resolv.conf:/etc/resolv.conf:norbind"
                          "/etc/netns/${name}/nsswitch.conf:/etc/nsswitch.conf:norbind"
                          "/etc/netns/${name}/nscd-kill:/run/nscd:norbind"
                        ];
                      };
                    };
                  }) services
        ...
        networking.firewall.interfaces = builtins.listToAttrs (
          builtins.map (
            { name, veth, ... }:
            {
              name = "veth${name}host0";
              value = {
                allowedTCPPorts = (veth.hostPorts or [ ]);
              };
            }
          ) wgConfig
        );
